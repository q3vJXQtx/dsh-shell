...
![Build](https://github.com/q3vJXQtx/dsh-shell/actions/workflows/build.yml/badge.svg)

一个面向 **稳定性与可诊断性** 的 DSH（DeepSeek Harness）Windows 桌面外壳。

把 `dsh web` 提供的本地 Web UI 装进一个原生窗口里，并负责它的启动、就绪等待、
凭证获取、崩溃自愈与故障排查。
...
参考项目 [dsh-local-shell](https://github.com/masknull/dsh-local-shell) 的定位是对的，
但它的 `src-tauri/src/lib.rs` 是一个 **2030 行的单文件**，把进程管理、自愈、
窗口控制、token 抓取、配置读写全都塞在一起。这种结构直接导致了几类难查的故障
（其 CHANGELOG 中「卡在正在启动 / 白屏 / 死页」跨十几个版本反复出现）。
...
- **MSVC 链接器**：Visual Studio Build Tools 的 `VCTools` 工作负载（含 Windows SDK）

### 本机的两处特殊配置（不设就会失败）

这台机器的 Rust 装在 **D 盘**，且 MSVC 环境需要显式激活。直接敲 `cargo`
会撞上两类**看起来很吓人、实际与代码无关**的报错：

1. **`missing manifest in toolchain` / `no default is configured`**
   Git Bash 会话里没有 `CARGO_HOME` / `RUSTUP_HOME`，`cargo` 于是落到
   `~/.cargo/bin` 的 rustup shim，而 `~/.rustup/toolchains/` 下那份是
   **残缺的**（只有 cargo/clippy，没有 rustc，仅 65 MB）。
   完整工具链一直在 D 盘——**不要去重装工具链**。
2. **`LNK1181: 无法打开输入文件 kernel32.lib`**
   Git Bash 自带 `/usr/bin/link`（Unix 的硬链接工具），它会**抢占**
   MSVC 的 `link.exe`；即便抢对了，也还缺 `LIB` 环境变量。

两件事都由项目根目录的 `build.cmd` 一次性处理：

```bat
build.cmd check
build.cmd test --lib
build.cmd build --release --features custom-protocol
```

它做三件事：激活 `vcvars64.bat`（拿到 MSVC 的 `link.exe` 以及 `LIB`/`INCLUDE`）、
把 `CARGO_HOME`/`RUSTUP_HOME` 指向 D 盘、然后把参数原样转给 `cargo`。

> 路径是按本机写死的；换机器要改 `build.cmd` 顶部那几行。

### 命令

```bash
npm install
npm run tauri dev      # 开发（连 Vite dev server，带热更新）
npm run tauri build    # 打包（产出 NSIS 安装包）
```

### CI（GitHub Actions，无需本机编译）

- 推送到 `main` / 提 PR：自动跑 `tsc + vite build`、`cargo test --lib`
  （含 capability 守卫测试）、release 编译——全程在 `windows-latest` 上；
- 打 `v*` tag：CI 通过后自动产出 NSIS 安装包，并创建 **draft Release**，
  人工确认后发布。

### ⚠️ 不要直接 `cargo build --release`

`tauri` 的 `build.rs` 是这样判定开发/发布模式的：
...
也就是说，**`--release` 这个编译参数并不决定运行模式**。如果直接用
`cargo build --release`（没有同时打开 `tauri` 的 `custom-protocol` feature），
会得到一个「编译期是 release、运行期是 dev」的二进制：

- 它会去加载 `devUrl`（Vite 的 `http://localhost:1420`）；
- 没开 dev server 时，窗口就是**一片空白**，但日志里只有
  `控制台页面开始加载 | http://localhost:1420/` 这种看上去正常的记录；
- 前端没起来 → 不会调用任何命令 → `chat_load` 永远不会被触发。

正规做法就是走 `npx tauri build`（CLI 会自动打开该 feature）。
确实需要手工编译发布版时：
...
`Cargo.toml` 里已经声明了这个 feature，**但刻意不设成默认值**——
`tauri dev` 依赖 dev 模式才能连上 dev server，设成默认会让 `tauri dev` 直接失效。
...
| 日志 | 含义 |
|---|---|
| `控制台页面加载完成 \| http://tauri.localhost/` | 正常（发布模式，加载的是内嵌前端） |
| `控制台页面加载完成 \| http://localhost:1420/` | 跑成了 dev 模式却没开 dev server，见上一条 |
| 完全没有这两行 | WebView 根本没起来，看启动参数与 WebView2 运行时 |

---

## 运行时数据

应用是**绿色**的，全部数据放在可执行文件同级的 `dsh-shell-data/`：

- `settings.json` —— 用户配置
- `dsh-shell.log` —— 运行日志
- `diagnostics-<时间戳>.txt` —— 用户导出的诊断报告

删除该目录即可恢复出厂状态。token **只存在于内存中**，绝不落盘。

**唯一的注册表例外**：任务完成通知需要在
`HKCU\Software\Classes\AppUserModelId\com.dshshell.desktop` 下写两个值
（`DisplayName`、`IconUri`）。Windows 只为"已注册 AppUserModelID"的进程
显示通知，而正常途径是安装器建开始菜单快捷方式——本程序刻意不做安装器，
只能走注册表这条替代路。不写的话 toast 会被**静默丢弃**（不报错、不显示）。
卸载时删掉该键即可，没有其它残留。
