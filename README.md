...
- 🔗 **复用外部实例** —— 你自己先起了 `dsh web`？不用杀进程：失败面板点「复用实例（不重启）」借持久登录态直接进；要通知/自愈齐全随时「重启后端」切回托管
...
- **安装版** `DSH.Shell_<版本>_x64-setup.exe` —— NSIS，CurrentUser 安装、不需要管理员权限
- **便携版** `DSH.Shell_<版本>_x64-portable.exe` —— 单文件，扔进任意文件夹双击即运行，
...
**Q：端口被占用？**
诊断面板会识别占用者：上次遗留的 dsh（可接管）、外部启动的 dsh、还是无关进程（显示 PID）。
是外部 dsh 的话，失败面板首按钮是「**复用实例（不重启）**」——不动对方进程，
借 WebView2 的持久登录态直接进入（此模式无本代 token，任务完成通知不可用；
点「接管并启动」可切回全功能托管，或换个端口各跑各的）。
